...
struct DSU {
    ...
    DSU(int n) :n(n), _size(n, 1), _parent(n){
    ...
    int search(int u) {
        ...
        return _parent[u] = search(_parent[u]);
    }
...
void solve() {
    ...
    int j = 0;
    for(int i = 0; i < m; i++) {
        auto [q, ind] = queries[i];

        for(; j < n - 1; j++) {
            int w = edges[j].first;
            if (w > q) break;

            auto [u, v] = edges[j].second;
            --u, --v;

            dsu._union(u, v);
        }

        ans[ind] = dsu.get_pairs();
