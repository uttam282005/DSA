...
void solve() {
    ...
    long long numOfNodes = 1; // k^(n - 1)

    for(int i =  1; i <= n - 1; i++) numOfNodes *= k;

    vector<pii> st;
    vector<int> nextEdge(numOfNodes + 1);
    vector<int> eulerPath;
    st.push_back({0, -1});

    while(!st.empty()) {
        int cur_node = st.back().first;
        int digit = st.back().second;
        if (nextEdge[cur_node] < k) {
            int d = nextEdge[cur_node]++;
            int next_node = (cur_node * k + d) % numOfNodes;
            st.push_back({ next_node, d });
        } else {
            if (digit != -1) {
                eulerPath.pb(digit);
            }
    ...
    string result = "";
    for(int i = 1; i < n; i++) result += alphabet[0];
    for(int d : eulerPath) result += alphabet[d];
