...
void solve() {
  ...
  while(q--) {
    int src, des;
    cin >> src >> des;
    int cycle_entry = jump(src, len[src]);
    if (jump(src, len[src] - len[des]) == des) {
      cout << len[src] - len[des] << endl;
    } else if (jump(cycle_entry, len[cycle_entry] - len[des]) == des) {
      cout << (len[cycle_entry] - len[des])
    }
  }
