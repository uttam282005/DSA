...
vector<int> getLRV(vector<int> &a, int l, int r) {
    int mul = 1;
    for(int i = l; i <= r; i++) {
        mul *= a[i];
    }

    if (mul > 0) return {0, 0, mul};
    ...
    int remLeft = mul / leftMul;
    int remRight = mul / rightMul;


    if (remLeft < remRight) {
        return {0, right, remRight};
    }

    return {left , 0, remLeft};
}
