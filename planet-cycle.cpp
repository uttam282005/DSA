...
int jump(int src, int dis) {
    for(int i = 0; i < LOG; i++) {
        if (dis & (1 << i)) src = up[src][i];
    }
...
bool isPartOfCycle(int node, int cycle_entry) {
    return jump(cycle_entry, len[cycle_entry] - len[node]) == node;
}
...
void solve() {
    ...
    for(int i = 1; i <= n; i++) {
        int cycle_entry = jump(i, len[i]);
        if (isPartOfCycle(i, cycle_entry)) {
            cout << len[cycle_entry] << " ";
        } else {
            int l
                = 1;
            int r = n;
            ...
            while(l <= r) {
                mid = (l + r) / 2;
                int node = jump(i, mid);
                if (isPartOfCycle(node, cycle_entry)) jr = mid, r = mid - 1;
                else l = mid + 1;
            }

            cout << jr + len[cycle_entry] << " ";
        }
    }
