...
bool vis[N];
void dfs(int node, vector<vector<int>> &G, stack<int> &st) {
    vis[node] = true;
...
void dfs2(int node, vector<vector<int>> &G, int scc, vector<int> &scss) {
    scss[node] = scc;
    vis[node] = true;

    for(int child : G[node]) {
        if (!vis[child]) {
            dfs2(child, G, scc, scss);
        }
    }
...
void solve() {
    ...
    while(!st.empty()) {
       ...
       st.pop();
       if (!vis[node]) {
       dfs2(node, GT, scc, scss);
       scc++;
       }
    }

    cout << scc - 1 << endl;
    for(int i = 1; i <= n; i++) cout << scss[i] << " ";
}
