...
void solve() {
    ...
    vector<array<long long, 2>> dp(n + 1, {-1, -1});

    auto dfs = [&](auto&& self, int node, int parent, int side) -> long long {
        if (dp[node][side] != -1)
            return dp[node][side];
        ...
        for (int child : G[node]) {
            ...
            ans += max(
                self(self, child, node, 0) +
                    abs(ranges[node][side] - ranges[child][0]),

                self(self, child, node, 1) +
                    abs(ranges[node][side] - ranges[child][1])
            );
        }

        return dp[node][side] = ans;
    };
