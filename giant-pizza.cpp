...
void dfs(int var, stack<int> &st) {
   ...
   for(int child : G[var]) dfs(child, st);

   st.push(var);
}

void dfs2(int var, int sec) {
    component[var] = sec;

    for(int child : GT[var]) {
        if (component[child] == -1) dfs2(child, sec);
    }
}
...
void solve() {
    ...
    for(int i = 1; i <= 2*m; i++) component[i] = -1;

    for(int i = 0; i < n; i++) {
        char opinion1, opinion2; int topling1, topling2;
        cin >> opinion1 >> topling1 >> opinion2 >> topling2;
        if (opinion1 == '-') topling1 = negateVar(topling1);
        if (opinion2 == '-') topling2 = negateVar(topling2);
        G[negateVar(topling1)].pb(topling2);
        G[negateVar(topling2)].pb(topling1);

        GT[topling2].pb(negateVar(topling1));
        GT[topling1].pb(negateVar(topling2));
    }
    ...
    vector<int> ans(m + 1);
    for(int i = 1; i <= m; i++) {
        ans[i] = component[i] > component[negateVar(i)];
    }
